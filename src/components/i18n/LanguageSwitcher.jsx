  ...
  return (
    <div ref={ref} className="relative select-none">
      <button
        ...
        className={`flex items-center justify-between gap-2 rounded-lg border transition-all duration-200 w-full min-w-25
          /* Mobil uchun kattaroq padding (px-4 py-2.5), katta ekranlar uchun asl (px-3 py-2) */
      ...
      >
        {/* Bayroq va yozuv o'lchamlari ham mobil uchun biroz kattalashtirildi */}
        <span className="text-xl sm:text-lg">{current.flag}</span>
        <span className="font-semibold text-base sm:text-sm">
          {current.label}
        </span>
        <span
          className={`text-xs sm:text-[10px] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        >
          ▼
        </span>
      </button>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-2 w-full min-w-30 bg-white dark:bg-[#2a2a2e] rounded-xl border border-gray-200 dark:border-white/10 shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          {LANGS.map((l) => (
            ...
            >
              <span className="text-lg sm:text-base">{l.flag}</span>
              {l.label}
            </button>
          ))}
        </div>
